(ns pressops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (PressAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the PressGovernor (:govern) and the rollout
  ...
  human operator/managing editor. The approver resumes with
...
(defn build
  ...
  `pressops.store/Store`).
  opts:
    :advisor      -- a `pressops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PressAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; PressGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :publication-id (:publication-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :publication-id (:publication-id request) :by (:by approval)}]}
            {:disposition :hold
